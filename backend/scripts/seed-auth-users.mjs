...
async function seed() {
  ...
  for (const u of usuarios) {
    ...
    const jaExiste = existentes?.users?.find((x) => x.email === u.email);

    if (jaExiste) {
      // Atualiza senha e metadata (idempotente)
      const { error } = await supabase.auth.admin.updateUserById(jaExiste.id, {
        password: u.senha,
        email_confirm: true,
